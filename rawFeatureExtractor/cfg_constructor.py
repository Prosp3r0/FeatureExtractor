...
def getCfg(func, externs_eas, ea_externs):
	...
	for bl in control_blocks:
		...
		#print control_ea, 1
		refs = idautils.CodeRefsTo(start, 0)
		for ref in refs:
		...
		#print control_ea, 1
		refs = idautils.CodeRefsTo(start, 1)
		for ref in refs:
			if ref in control_blocks:
				dst_node = control_blocks[ref]
				if dst_node not in visited:
...
def getBinaryValue(node):
	start = node[0]
	inst_addr = idc.next_head(start)
	value = 0
	addr = 0
	for x in xrange((inst_addr - start)-1):
		addr = start + x
		y = idc.get_original_byte(addr)
		print value, addr, y
		value = value | y
		value = value << 8
		print value
...
def cfg_construct(func):
	...
	for bl in seq_blocks:
		...
		src_node = (start, end)
		if end in seq_blocks and idc.print_insn_mnem(idc.PrevHead(end)) != 'jmp':
